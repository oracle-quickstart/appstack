...
resource "oci_identity_api_key" "user_api_key" {
  #Required
  key_value = (var.use_existing_api_key ? file(var.api_key) : tls_private_key.rsa_api_key[0].public_key_pem)
...
resource "local_file" "api_private_key" {
  ...
  content = (var.use_existing_api_key ? file(var.api_key) : tls_private_key.rsa_api_key[0].public_key_pem)
  count = (local.use-image ? 0 : 1)
}
